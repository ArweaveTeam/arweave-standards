...
It is possible to construct data-items in such a way that they themselves are a bundle.
The nested bundle and its nested data-items need to be verified and validated in the same manner
as the non-nested bundle.

Gateways should treat data-items as a nested bundle if the tag-value pairs match the specification
described in [section 1.1](#### 1.1 Transaction Tags) and the data-item data is an array-buffer of
a Bundle matching the format described in [section 1.2](#### 1.2 Transaction Body Format) contianing
verifiable data-items using the method described in [section 2.1](#### 2.1 Verifying a DataItem).

Query of data-item's transaction headers should contain a parent field (a.k.a bundledIn) that indicates
parent-child relationship of the data-item to it's immediate parent. Any nested bundle should be trace-able
to a layer-1 transaction by recursively following the parent field.
